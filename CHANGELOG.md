...
### 已知限制与已知问题
- **relay 侧三个哨兵签名与住宅黑名单的自动学习，在当前的 selector 拓扑下不工作 —— 4.1.x 修**。`relay_upstream_error` / `relay_upstream_auth_failed` / `relay_google_blocked` 这三条签名，以及「从 relay 日志里学出该拉黑哪个上游-域名组合」那套自动学习，判据都只认「错误行里带着上游成员自己的 tag」这种形状；而 relay 的路由规则指向的是选择器分组（每槽一个 + 全局池），sing-box 在这种拓扑下只把**外层分组**的 tag 写进错误行，成员的 tag 根本不出现。两台生产机 14 天的日志取样证实：自从 relay 换成按槽分组的拓扑之后，**没有一条真实错误行能被这两处解析到**。**如实说：这不是「偶尔漏」，是全盲。** 顶上的只有住宅体检那条每 120 秒一轮的主动探测（面窄、比哨兵慢一个数量级）；**黑名单自动学习连兜底都没有**，只剩每天那 12 个固定探针域名 —— **运维不要以为它还在正常学习**。这个盲区**不是 4.1 引入的**（全局池那一层从一开始就是分组、一直都没被认过，4.1 的按槽分组只是同一个问题的第二个实例），所以不阻塞 4.1.0；修法已有设计方向，列为 **4.1.1 的第一个设计项**。
