...
- `bui hy2-prestart <配置>` 不再误删**正在运行**实例的端口跳跃规则：它按「规则指向本实例 base 端口」认领，所以对运行中的实例执行会把该实例**现役**的那张 nft 表 / iptables 链一起删掉——端口跳跃当即失效，且要等该实例下次重启才恢复（这条命令本来只给单元启动前的 `ExecStartPre=` 用）。现在手跑时先查该实例的单元状态，恰为 `active` 就拒绝执行、**一步清理都不做**并以退出码 2 退出，提示想恢复规则改用 `systemctl restart <单元>`；确实要在运行中清理（例如实例卡在 failed）加 `--force`。systemd 的正常启动路径不受影响（执行 `ExecStartPre=` 时单元是 `activating` 而不是 `active`），watchdog 的崩溃循环自愈也照旧清理。
