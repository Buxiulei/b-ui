...
- 删住宅上游时点名**必须重新获取订阅**的用户：槽位变动会改 HY2 住宅节点的端口（`40000+槽序号`）与端口跳跃区间（`41000-50000` 按槽位空间等分），这两样都写死在已下发的订阅里、客户端不会主动发现。`bui residential remove` 与面板在删除后列出受影响的用户名（只列手里那份订阅真的不能用了的人：端口变了，或旧跳跃区间不再整段落在本槽的新区间内），同一份名单落一条事件（签名 `resi_slot_port_changed`），`bui incidents` 与面板「事件」卡都能回看；删上游的两个端点回包追加 `resubscribe` 字段。
