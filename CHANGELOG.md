...
只改守护进程的日志文案、一条住宅告警的清除与读日志的方式，**不动渲染器、不动订阅、不动内核配置**：升级前后三种订阅的字节不变。

### 修复
- **`b-ui.service` 的内存被 `journalctl` 的页缓存顶满**：2026-09-24 在 bwg-tizi 实测 `b-ui.service` 的 `memory.current` = 206.7 MB ≈ `MemoryMax=200M`，其中 `file` 201 MB（`inactive_file` 182 MB）、`anon` 只有 3.8 MB；`memory.events` 的 `max` 计数每分钟涨几百到两万多次，`memory.pressure` full avg300 = 3.9%，`sock_throttled` = 510，同一时段有一次 Clash API 请求「error sending request」失败。bwg-rick（journal 390 MB）上 b-ui 的 `file` 是 118 MB，还没顶满。根因：守护进程读日志时派生的 `journalctl` 是它的子进程、在同一个 cgroup 里，`journalctl` mmap 的 journal 页第一次被读到就记到 b-ui 头上——在独立单元里单独量过，哨兵游标失效时那种读法（`-o json -u <六个单元> --since @<现在-10s>`）冷读一次记 **49 MB**，黑名单游标失效时那种（`--since -25h`）记 36 MB；攒到上限以后守护进程每次分配内存都要先在上限处回收页缓存，回收压力和 socket 节流都落在守护进程自己头上。4.1.2 发布后观察项里说「`memory.current` 常年贴着 `MemoryMax` 是可回收的 page cache，不是泄漏」，这一点没错，但顶着上限运行本身就有代价。修法：哨兵、证书重载确认（这两处经 `journal_read`）和黑名单候选学习（`journal::collect`）读日志时，`journalctl` 改用 `systemd-run --wait --pipe --quiet --collect -p MemoryMax=128M -p LogLevelMax=warning` 放进一个临时 service 单元 `bui-journal-<pid>-<序号>` 里跑。这样页缓存记在那个单元名下，单元回收后归到上级 slice；`MemoryMax=200M` 现在只量守护进程自身，值不变。`journalctl` 的退出码、stdout、stderr 对调用方不变，游标失效仍然是非零退出、调用方照旧丢掉游标。机器上没有 `systemd-run`，或者从 stderr 看出是 `systemd-run` 自己没起来时，回落成直接执行 `journalctl`，每个进程只记一次 debug。**没有用 `systemd-run --scope`**：scope 不接受 `LogLevelMax=`，PID 1 每起一个 scope 就往 journal 写一行「Started …scope」，哨兵每 2 秒读一次，一天就是四万多行，而且写进的正是要读的那份 journal；临时 service 设了 `LogLevelMax=warning` 以后，成功读一次一行都不写（开发机 systemd 255 实测），读失败时只写一行「Failed with result」。代价是 PID 1 每 2 秒要起、收一个临时单元（开发机 root 系统级实测每次 40 ms、PID 1 零日志行、无残留单元；两台生产机升级后盯 `systemctl show b-ui -p CPUUsageNSec` 的增速与 `b-ui.service` cgroup 的 `file` 是否回落）。`watchdog` / 对账里取单元最后几行的 `journalctl -n` 和菜单的查看日志没有改（都是只读结尾、很少触发）。
- **`journalctl -o cat` 看不到 WARN 的失败原因**：`tracing-journald` 会把 `error = %e` 写成一个独立的 `F_ERROR` 字段，`MESSAGE` 里只有固定文案。生产上「用户同步有失败项，下一轮安全网会重试」这一行看不出原因（实际是「住宅 HY2 门位读不到（GET /proxies）：Clash API 不可达」），只有 `-o json` 才看得到。`tracing-journald` 0.3.2 没有把字段拼进 `MESSAGE` 的开关，自己写一个 Layer 又会改变所有日志行的形状（哨兵按 `MESSAGE` 匹配），所以改成逐条处理：`warn!` / `error!` 级别、带 `error` 字段、文案里有「失败」「忽略」「重试」的 45 处，都在文案末尾加上「：{原因}」，`error` 字段保留。`USER_SYNC_FAILED_LOG` 常量不变，这一行现在是「{常量}：{原因}」；哨兵本来就是按包含匹配，已补用例证明带上原因后仍然认得出。
- **「全部住宅上游探测不达标，出口已降级但未切换」告警清不掉**：巡检规则 7 推了这条全局告警之后，没有任何代码会删它（`purge_stale_alerts` 只清上游级告警）。bwg-rick 的 `runtime.json` 里它挂了至少 7 天，同期 24 小时健康采样全绿。现在只要有一轮巡检出现健康的成员，就按精确文案把它删掉；没有这条告警时不写盘。
- **b-ui 收到 SIGTERM 退出时多打一条 WARN「runtime.json 落盘任务 panic（忽略）」**：这是退出过程中 tokio 取消了还没跑完的 `spawn_blocking` 落盘任务，不是 panic。现在取消只记 debug「runtime.json 落盘任务被取消（进程退出中）」，真的 panic 仍然记 WARN。
