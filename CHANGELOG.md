...
### 新增
- 菜单 [3] 导入之后，同一账号还剩别的重复节点时多问一句 `要合并吗？[y/N]`（默认 N）：答 y 把它们并进本次已更新的那条，答 n 全都留着，什么都不改。答 y 之后如果另一个 bui-c 操作还没结束，或者列表已经被别的会话改过，会分别提示「失败：另一个 bui-c 操作还没结束，没有合并，稍后再试」和「节点列表已经变了，没有合并」，两种情况下重复节点都原样留着。命令行 `bui-c import` 只提示、不问、不改：`同一账号还有 1 个节点与服务端这次给的端口或凭据不一致：…（本次已更新 …）`，`要合并请在菜单 [3] 里导入并答 y`。合并不动当前节点，也不重启代理。
