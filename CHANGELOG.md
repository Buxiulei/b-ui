...
- **住宅 HY2 每条 TCP 流少等 300 ms**：`hy2-residential.json` 的 `route.rules` 去掉了那条裸 `{"action":"sniff"}`。apernet 客户端（v2rayN 里那个）缺省 `fastOpen: false`、要等服务端回 `TCPResponse` 才发首包，而 sing-box 是在 route（sniff 就在这里）走完、出站也拨通之后才写 `TCPResponse` ⇒ 嗅探在等首包、客户端在等响应，**必然等满嗅探超时并且什么都嗅不到**。实测（25 次/格）：缺省 300 ms 那档首字节 307 ms、嗅到域名 0/25；`timeout:"50ms"` 是 56.6 ms、仍 0/25；`timeout:"1s"` 是 1010 ms、仍 0/25；不 sniff 是 5.6 ms。分流不受影响 —— 住宅 split 的关键字匹配一直靠 relay 自己那条 `rules[0]` sniff（sing-box 的 sniff 动作不改写连接目标，住宅内核这一侧嗅到的域名原本就传不到 relay），面板与 CLI 的连接归户只读 `/connections` 的 `rule` / `chains` / `id`，`metadata.host` 一处都没读。**受影响的只有住宅 HY2 节点，直连 HY2（apernet 服务端，`sniff.rewriteDomain`）本来就没这个代价。**
