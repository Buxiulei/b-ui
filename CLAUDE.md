...
2. **`crates/bui`** — 服务端单二进制：`install / upgrade / serve / reconcile / status / import-v3 / auth-hook / residential / menu / harden-ssh`。期望态存 `/opt/b-ui/state.json`，对账器把文件/单元/sysctl/防火墙/二进制拉到期望态（非受管项只报不改）；面板 API + 内嵌前端 + 住宅巡检 + 黑名单 + 证书监听 + watchdog + 升级都在这一个进程里，**没有 cron、没有独立 timer、没有改配置的 shell 脚本**。
