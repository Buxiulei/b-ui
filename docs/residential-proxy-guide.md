...
### 8.1 住宅 HY2 的凭据池与门（4.1）

住宅 HY2 从 4.1 起是**一个** sing-box hysteria2 入站 `:40000`（整段跳跃 `41000-50000` 由
`table inet bui` REDIRECT 进去）+ 一池静态凭据，每条凭据在内核里有一个门（selector）：

```bash
bui residential pool status          # 已用 / 容量 / 空闲 / 第几代 / 持有人（只有用户名）
bui residential pool grow --to 64    # 手动扩容（幂等；不带 --to 就补到按人数算出的目标）
```

- 容量 = `clamp(ceil16(2 × 住宅 HY2 用户数), 32, 256)`，空闲凭据是**预留的门位**；
  空闲不足 20% 时 `bui status` 会点名「下次建用户会当场扩容 ⇒ 重写配置 + 重启住宅内核 ⇒
  全体住宅 HY2 会话重连一次」。端点与 CLI 都**不打印凭据 id 与密码**，只给数字与用户名。
- 用户的全部生命周期动作（建号 / 到期 / 封禁 / 停用 / 轮换凭据）都只是切一次门，
  `hy2-residential.json` 一个字节都不动、内核不重启，槽位表与出口 IP 也不变。
- 住宅 HY2 的密码是这条池凭据（`{name}:{secret}`），**与直连的 HY2 密码各自独立**：
  面板「重置订阅链接与凭据」换的是直连密码 + 新发一条住宅凭据，两者此后互不影响。

**到期 / 封禁在住宅 HY2 上的表现（重要，排障常问）**：门被切到 `deny` 之后，
**客户端仍会显示「已连接」，但所有请求都被拒绝**——握手照旧成功（TLS + QUIC 都通），
只有流量落到那条打不通的出站上。直连节点则是**建连时**就被拒。所以「用户说还连着但打不开
网页」的第一判据是看他的门位：面板用户列表的槽位列有门位徽标（`slot-<i>-out` = 放行、
`deny` = 已停用/已到期/超限、`未分配` = 还没拿到凭据，订阅里也没有住宅 HY2 节点）。
住宅侧**鉴权失败没有日志**（sing-box 不打），能看到的只有成功那一行
`[<凭据名>] inbound connection to …`，所以别去 journald 里找「拒绝」记录。

**「重置订阅链接与凭据」（rotate）之后要通知 Linux 客户端用户「重新导入**并切换**」**：
rotate 会释放这个人原来那条住宅凭据（门当场切 `deny`）、另发一条新的，而池里新发凭据的名字是
`rNNN`（迁移用户原来那条叫他自己的用户名），所以**住宅 HY2 的账号名也换了**。对按账号匹配的
`bui-c` 来说这就是另一个账号：重新导入只会**新增**一条新的住宅节点，旧的那条留在原地不动；
旧凭据的门已经是 `deny` ⇒ 那条旧节点**显示已连接但所有请求被拒**，它正好是当前活动节点时，
用户的体验就是「连着但什么都打不开」。rotate 同时也换 VLESS UUID，所以**有 Reality 权益的用户
Reality 那两条同样各新增一条**（uuid 变了就是另一个账号），旧的两条从此连不上。

所以 `sudo bui-c import --sub -` 或菜单 [3] 导入完之后**必须明确切到新的住宅 HY2 节点**，而且
**不能只靠导入完那一问**：菜单 [3] 问的「切换到新导入的 …？」是**第一个**新节点，融合权益
（Reality + HY2）的用户那通常是 **Reality 直连**那条 —— 答 y 只会切到它，住宅出口被静默丢掉。
要么答 n 再用菜单 [1] 选节点，要么命令行 `sudo bui-c switch <新的住宅 HY2 节点名>`
（`bui-c import` 不替你切，`--activate` 切到的也是这批导入的第一个节点）。新节点的名字以客户端
列表里显示的为准（命名规则在 `bui-c` 侧，见 [docs/HANDOVER-bui-c.md](HANDOVER-bui-c.md) §6）。
切过去之后，旧节点用菜单 [6] 删掉。
这条只影响 rotate，换槽 / 增删住宅 IP 都不用重导（端口与区间与槽位无关）。
