...
- 判据①' 首条 relay 错误 → 事件 ≤ `DRILL_NOEXIT_SLA`（新常量，缺省 25 秒）：轮询 ≤2 + 原上游快探 ≤3 + `BORROW_PROBES_PER_CALL`(3) ×（Clash PUT + 单次验证 ≤ `BORROW_VERIFY_BUDGET_SECS` 4 秒）+ 收尾 PUT ≈ ≤18 秒，留出等第 2 条错误的余量取 25。丢包只造得出「网关 TCP 连不上」那条快路（每次验证 ≤3 秒），「网关活着、隧道卡住」那一段演练量不到，靠代码里的 `timeout` 兜住 + `slots` 那边的假件单测。
