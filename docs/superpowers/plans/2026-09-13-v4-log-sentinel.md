...
**Goal:** 主理人 2026-09-13 原话：「bui 需要监听后台日志，如果出现报错，要立马处理，比如某个住宅代理 ip 连不通了，就要启用预案，立马分配新的 ip」。本计划在守护进程里加一个**日志哨兵**：每 2 秒（演练后由 5 秒改，见 D17）增量读受管单元的 journald，按签名表识别故障，经去抖 / 冷却后执行预案（带外快探 → 该槽立即借用健康 IP / 用户同步重试 / 告警），每次触发写一条事件（`runtime.incidents`，环形 200 条），`bui status` 显示最近 5 条、`bui incidents` 查全量、面板新增「事件」卡；外加一个在生产机上用 iptables 断一条上游的真机演练脚本。
...
- **D1 采集 = 增量轮询，不是 `-f`**：与黑名单的 `residential::journal::collect` 同一思路（契约决策 §E），每 `POLL_SECS = 2` 秒（演练后由 5 秒改，见 D17）跑一次 `journalctl --no-pager -q -o json -u <每个单元> (--after-cursor <c> | --since @<unix 秒>)`。游标**以内存为准**：每轮都从上一轮最后一条的 `__CURSOR` 续读；落盘到 `runtime.extra["sentinel"].cursor` 有节流（有事件 / 冷却表变化时立即落；只是游标前进则至少隔 `CURSOR_PERSIST_SECS = 60` 秒落一次）——hysteria 在 info 级每条连接都写日志，若每 2 秒 fsync 一次 `runtime.json` 一天就是四万三千次写盘。代价：守护进程重启后最多重读 60 秒日志，由 D12（窗口按日志时间戳）与持久化的冷却表兜住，不会重复动作。**首次启动（没有游标也没有 `since`）从「现在」读起，不回放历史**；游标失效（日志轮转、`journalctl` 非零退出）同样丢游标、`since = 现在`。
...
- **D4 relay 签名的边界**：`open connection to <host>:<port> using outbound/<http|socks>[resi-N]: <reason>` 里，`reason` 是 `unexpected status: 4xx/5xx`（407 除外）或 `socks5:` 开头的 REP 拒绝 ⇒ 这是「上游拒绝了**这个目标**」，归黑名单（spec §5.4），哨兵**不计**；「上游本身不能用」分两类（演练后拆开，见 D17）：`connection refused` / `i/o timeout` / `deadline exceeded` / `no route to host` / `network is unreachable` ⇒ 连不上，计 `relay_upstream_error`（60 秒 ≥2 条）；`unexpected status: 407` / `proxy authentication` / `incorrect user name or password` ⇒ 凭据失效，计 `relay_upstream_auth_failed`（60 秒 ≥3 条，同一预案、共享动作冷却）；`403` 且（含 `serp` 或目标是 Google 搜索域名）⇒ `relay_google_blocked`。成员 tag 是位置键，匹配当场经 `clash::id_of_tag` 换成 uuid 再计数（契约决策 §C），换不出来的丢弃。误判的代价只是一次带外探测（D5 是仲裁者）。
- **D5 带外探测用快探 `health::probe_quick`，不用巡检的 `probe_member`**：`probe_member` 在「上游被丢包」时要走完 `timed_get`（8s）+ `get`（10s）+ 407 补判 CONNECT（10s）+ Google（8s）+ STUN（5s），约 40 秒，达不到演练的 15 秒。快探先在 `PROBE_TCP_TIMEOUT_SECS = 3` 秒内对网关建 TCP（`Prober::gateway_tcp_within`：解析出的各地址并发拨、整体限时，演练后改，见 D17），**连不上直接判不可达**；连得上再走一轮与巡检同口径的 `probe_reachable`（含 407 补判，单次超时 `ReqwestProber::with_timeout(PROBE_TIMEOUT_SECS = 5)`）。不测 Google / UDP / 测速。
...
- **D7 借用 = `slots::borrow_now`**：与 `drive_slots` 同一口径、只做「借出」一半——只动**当前出口就是故障 IP** 的槽（本槽 IP 就是它、或正借用它；`current_upstream_id = None` 视为停在本槽 IP 上）；手动 pin 的槽不动；目标 = 本槽 IP 不是故障 IP 且健康、Google 未封 ⇒ 回本槽，否则 `health::rank_healthy` 里排名最高的非故障健康 IP；一个都没有 ⇒ 保持现状（fail-open）；被挪的槽 `back_rounds` 归零。**不推进任何槽的 `back_rounds`**——直接再调一次 `drive_slots` 会让恢复中的槽多数一轮，把「连续 3 轮」的防抖打穿。PUT 助手 `put_slot` 从 `drive_slots` 抽出、两处共用。**与巡检互斥**：`drive_slots` 整轮「读快照 → 逐槽 PUT → 按快照写回 `current_upstream_id`」，`borrow_now` 若落在中间，它写的记录会被那轮写回盖成旧值（relay 的 selector 已经切走，面板与演练脚本看到的却是没借用，要等下一轮巡检才自愈），所以两者共用 `slots.rs` 的私有锁 `SLOT_SWITCH`（`tokio::sync::Mutex<()>`，各自整段持锁；**锁内不只有毫秒级的 Clash PUT** —— D18 之后 `borrow_now` 在锁内最多做 `BORROW_PROBES_PER_CALL` 次带预算的快探，一次持锁最坏「每个受影响的槽几次 PUT + 3 × `health::QUICK_PROBE_BUDGET_SECS`」，巡检那一轮只是排队等，两者都用 `MissedTickBehavior::Delay` 不补跑）。**不动全局 `resi-pool`**：哨兵只挪每槽的 `slot-<i>-pool`；全局 selector（relay 里 `dns_resi` 的 detour 与 global 模式的 `route.final` 用它）仍由巡检驱动。故障 IP 恰好是全局选择时，要等下一轮巡检（≤ 2 分钟）才切走；这段时间里受影响的主要是经 `dns_resi` 解析的关键字域名查询，各槽入站的连接走本槽 selector，已经被哨兵挪走。**2026-09-14 补（D18）**：`put_slot` 成功只说明 Clash 收下了这次切换，不说明借到的那条此刻能用——候选取自上一轮巡检的 `runtime.health`，最坏情况已过期。所以借用后要对刚切过去的那条跑一次同口径的 `health::probe_quick`（同一个函数、同一个 `prober`、同一个 `PROBE_TCP_TIMEOUT_SECS`，**逐条按 `host:port` 探、不按网关主机归组**），整体限时 `health::QUICK_PROBE_BUDGET_SECS = 4` 秒、结论三值：确认可用即借到，确认不可用就把这一条 `mark_unhealthy` 再试下一个，预算内未能确认就保留这个候选并如实报「未确认」；验证次数的上限是**单次调用**的 `BORROW_PROBES_PER_CALL = 3`（跨槽共享）；候选逐条被证死才把 selector 放回本槽自己的上游（不补探它）并如实报「当前无可用出口」。与 `drive_slots` 的「切回要连续 3 轮」故意不对称：借用判错是静默失败（用户没网、三处界面都说已自愈），切回判错是吵闹失败（来回抖动本身就是信号）——**失败会静默的方向要即时证据，失败会吵闹的方向要持续证据**。
...
- **D18 借用后带外验证与「无可用出口」的终态（2026-09-14，含当天审查后的修订）**：见 D7 末尾与 Task 10 的 2026-09-14 修订。口径：① 借用后用与判「原上游坏」**同一个** `probe_quick` 验证刚切过去的那条；② 验证**逐条按 `host:port`**，绝不按网关主机归组——生产池里多条上游是同一网关的不同静态端口、每个端口一个出口 IP，归组会把整组判死、直接砸掉「住宅 IP 连不通立刻分配新 IP」这个功能，也会让演练判据②③必然失败；③ 验证有**整体预算** `health::QUICK_PROBE_BUDGET_SECS = 4` 秒（`tokio::time::timeout` 包住整个 `probe_quick`，即 `health::probe_quick_within`），结论**三值**：确认可用 ⇒ 与没有这次验证时逐字一致（`switched`、无 note、「槽 i 已临时切到 Y」）；确认不可用 ⇒ 把**这一条** `mark_unhealthy`、试下一个候选；预算内未能确认 ⇒ **保留这个候选**、不记不健康、事件写「已切到 Y，未能在 4 秒内确认可用」（本槽原来的出口是已知死的，**未知优于已知死**；巡检下一轮用完整预算复核）。4 秒的来历：网关 TCP 连不上那条 ≤3 秒返回，网关活着那条正常一次 GET 远小于 1 秒；**必须 > `PROBE_TCP_TIMEOUT_SECS`**（`sentinel::run` 有一条用例钉住这个不等式：调反了每条「网关连不上」的上游都从「确认不可用」退化成「未确认」，借用侧就在第一个候选上停住 —— 既不记不健康也不试下一个，候选轮换静默失效；判原上游那侧虽然仍会动作，也丢掉了「凭据失效」与「不可达」的区分）；**必须有这个上界**——`probe_quick` 在网关活着时会继续走 `probe_reachable`（`timed_get` 吃 `LATENCY_PROBE_TIMEOUT_SECS` = 8 秒、**不是** prober 的 5，加 `get` 5、加 407 补判 5，多地址网关还按 `dial` 逐地址串行），审查用生产参数对着「accept 后永不应答」的监听实测单次 **28.4 秒**，而这正是本改动主打的故障形态；**已知取舍**（2026-09-14 审查第 4 条）：4 秒对真实住宅出口偏紧 —— 经隧道首包 1–3 秒并不罕见，加上最坏 3 秒的网关 TCP 就可能越过 4 秒，于是**健康**候选也会常态报「未确认」；代价按侧不同：借用侧 fail-safe（保留候选、不记不健康），文案从「已临时切到 Y」退化成「已切到 Y，未能在 4 秒内确认可用」；判原上游那侧会多借一次兄弟 IP（吵闹，恢复后第 4 轮巡检才切回，约 8 分钟，见 D6），换来「隧道挂死」这条路上仍然 15 秒自愈。要不要放宽**等 bwg-tizi 上实测一次真实借用验证的耗时再定**；**2026-09-14 审查第 1 条的修订**：这个预算原先只包住借用**后**那次验证，哨兵判「原上游坏了」的那次是裸的 `spawn_blocking`、一点时限都没有 —— 两条 SLA 在「网关活着、出口 IP 死了」这个最常见形态上必破。现在两处共用 `health::probe_quick_within` 这一份（同一个预算、同一套三值），判原上游那侧「没能确认」⇒ **按不可用处置**（2026-09-14 审查第 2 条的裁决）：`probe_reachable` 的第一次请求 `timed_get` 吃 `LATENCY_PROBE_TIMEOUT_SECS` = 8 秒 > 4 秒预算，所以「网关活着、隧道挂死」这条路上 `Dead` **算术上不可达**，而触发本签名的正是 relay 的 `i/o timeout` / `deadline exceeded` = 隧道挂死本身 —— 原先的「不动作」会让哨兵在主打形态上恒定空操作，只能等巡检 ≈4–5 分钟（`HEALTH_INTERVAL_SECS` 120 × `FAIL_TO_UNHEALTHY` 2 轮迟滞，且挂死时每轮 `probe_member` 自己要 ~40 秒），不是自述里「偶发报未确认」那么轻。改判的依据：relay 已在 60 秒内独立报了 ≥2 条错误、带外也过不去，两者**互为佐证**（不是「无证据」）；巡检那边本来也把同一个超时当失败证据（`apply_hysteresis(ok = false)`），改后不再自相矛盾；代价不对称 —— 误判死 = 多借一次兄弟 IP（吵闹：按 D6，`mark_unhealthy` 之后巡检要先连续 `OK_TO_HEALTHY` = 2 轮探通重新判健康、再攒满 `SLOT_BACK_ROUNDS` = 3 轮，恢复后第 4 轮才切回，约 8 分钟），不动作 = 用户静默断网几分钟。文案如实写「没能在 4 秒内确认可用（网关通但隧道无响应，或探测任务异常），加上 relay 刚连报错误，按不可用处置」，级别 Error（真动了手，照常占 10 分钟冷却）。**借用后验证那一侧不跟着改**：那条候选没人报过错，没有这层佐证，「未能确认」仍是保留候选、不记不健康 —— 两处**观测**共用 `probe_quick_within` 这一份，**处置**故意不对称；④ 上限是**单次调用的总验证数** `BORROW_PROBES_PER_CALL = 3`、跨槽共享（`borrow_now` 按槽循环，槽级上限会让单次调用最坏做「池大小 − 1」= 7 次），调用内缓存「已证死 / 已确认可用」不重复探；预算用尽后剩下的槽照样 PUT 到各自的最优候选、按「未确认」报，**不许**退回「保持现状」把槽留在已证死的上游上；⑤ 候选**逐条都被证死**时把 selector 放回本槽自己的上游并如实报「候选 X、Y 都探不通，当前指向本槽 IP Z，当前无可用出口」，连这次 PUT 都失败时把这一层也说出来；候选被「本次调用已证死」剪空的槽走同一条口径（证据已经拿到了，不能让它停在刚被证死的故障 IP 上），而一开始就没有候选仍是 fail-open「保持现状」；⑥ 告警文案一律按「谁失败、失败在哪一步、现在指向谁」写，`已临时切到` 只许出现在「确认可用」那一种上，`无可用出口` 只许出现在⑤那一种上（中途 PUT 失败停下来时剩余候选没探过，不许说）。处置延迟判据因此拆成两条（按真实数字重算，2026-09-14 主会话裁决）：**有可用出口 ≤15 秒** = 等第 2 条错误 ≤5（中继拨号超时）+ 轮询 ≤2 + 判原上游快探 ≤4（预算）+ PUT + 首候选验证（健康时 < 1）⇒ **≈ ≤12 秒**（原先写的「两次快探都撞满各自的 4 秒预算 ⇒ 正好 15 秒」删掉：首候选没能确认可用的那条路不是「有可用出口」，不进这条算式）；**无可用出口 ≤25 秒** = ≤5 + ≤2 + ≤4 + 3 ×（PUT + 验证 ≤4）+ 收尾 PUT ⇒ **≈ ≤24 秒**（2026-09-14 审查第 3 条：验证那一项要取到三个**满额**预算得落在**三个不同的槽**上 —— 同一个槽只要有一次撞满预算就 `break` 保留那个候选，往下试候选的 `Dead` 必然在 4 秒内返回，单槽只能逼近；方向是安全的高估，写明以免后来人按它反推单槽行为）。**PUT 按正常毫秒级计**：本机 Clash API（单次上限 `CLASH_TIMEOUT_SECS` = 2 秒），**Clash 自己挂起不在预算内**；次数也不是常数而按**受影响槽数**累加 —— 8 槽全压在故障 IP 上时约 12 次（第一个槽最多 4 次 + 1 次放回，其余 7 槽各 1 次），毫秒级下合计仍 < 1 秒。
- **D17 断连处置延迟（2026-09-13 bwg-rick @ rc9 演练判据①：首条错误 → 事件 30.3 秒，SLA 15 秒）**：原因——带外快探的 TCP 一步走 `ReqwestProber::dial`，**逐个地址各等满 5 秒**，而该上游网关是个解析出 6 个 IPv4 的域名、演练把它们全丢包，光这一步就是 6 × 5 = 30 秒（与实测吻合，演练的三个并发请求让门槛内的几条错误同一轮读到）；轮询 5 秒与门槛 3 条是另外两项固定成本。修订：① `POLL_SECS` 5 → 2（每轮仍是一个短命的 `journalctl --after-cursor`，不常驻进程）；② 连接类门槛 60 秒 3 → 2 条，凭据类单独成 `relay_upstream_auth_failed` 仍 3 条（同一预案、共享冷却），误报由带外探测兜底（探测通过 = Info，不借用、不占动作冷却）；③ 网关 TCP 改为 `Prober::gateway_tcp_within(up, PROBE_TCP_TIMEOUT_SECS = 3)`：解析与拨号都在后台线程、各地址并发拨、整体 3 秒，连不上即 `mark_unhealthy` + `borrow_now`，连得上才走原来的完整探测；巡检的 `probe_member` 不变。**新预算**（首条错误 → 事件，**已由 D18 修订**：借用后多了带预算的验证，且判原上游那次快探本身也有了 4 秒的整体预算 —— 「网关 TCP ≤3」只是它的快路，不是上界）= 等第 2 条错误 + 轮询 ≤2 + 网关 TCP ≤3 + 借用（毫秒级 Clash PUT，上限 `CLASH_TIMEOUT_SECS = 2`）。第 2 条错误：并发连接几乎同时报错 ⇒ ≈0，演练 ≈ 5–6 秒；客户端串行时最多一次 relay 拨号——sing-box `C.TCPConnectTimeout` = 5 秒（relay 出站不设 `connect_timeout`），单地址上游 ⇒ 5 + 2 + 3 + ≤1 ≈ 11 秒 ≤ 15。注意 sing-box 对域名上游也是逐地址串行拨（`N.DialSerial`），多地址网关上一条 relay 连接要 N × 5 秒才报错；这发生在「首条错误」之前，不计入判据①，但严格串行的单个客户端在这种网关上的第 2 条错误要晚到 N × 5 秒。
...
- Produces:
  - `sentinel::{POLL_SECS: u64 = 2, CURSOR_PERSIST_SECS: i64 = 60, PROBE_TCP_TIMEOUT_SECS: u64 = 3, PROBE_TIMEOUT_SECS: u64 = 5}`（`POLL_SECS` 由 5 改 2、`PROBE_TCP_TIMEOUT_SECS` 新增，均为演练后修订，见 D17）
...
判据口径（设计裁决 D6）：④ 的等待上限 660 秒 = `(OK_TO_HEALTHY 2 + SLOT_BACK_ROUNDS 3) × 120 秒 + 60 秒`——恢复后第 4 轮巡检切回，多给一轮对齐巡检相位。① 量的是「首条 relay 连接错误 → 事件的 `at`」，而 `at` 在快探与借用做完后才盖（Task 7），所以这段时长已含借用（2026-09-14 起也含借用后的带外验证）。

**2026-09-14 修订（借用后验证，见 D18）**：脚本加一个 `--all-ports` 模式。原来只封**一个**端口（一个出口 IP 挂掉，同网关的兄弟端口还活着），正好走「第一候选就通」的快路径；修完借用后验证之后，**最慢的一条路变成了「候选全不通」**，而判据①原先是在快路径上量的，所以判据拆成两条、并新增一个模式专门量慢路：
- `--all-ports`：把池里**所有**上游的端口一起丢包（`bui residential slots --json` 每槽一条上游 → `getent ahostsv4` 逐个解析 → 每个 `ip,port` 一条 iptables 规则），模拟整网关不可用；单端口模式与它共用同一套 `DROP_SPEC` / `drop_on` / `drop_off`，EXIT 的 trap 照样删净。
- 判据①' 首条 relay 错误 → 事件 ≤ `DRILL_NOEXIT_SLA`（新常量，缺省 25 秒）：等第 2 条错误 ≤5 + 轮询 ≤2 + **原上游快探 ≤ `health::QUICK_PROBE_BUDGET_SECS` 4 秒**（与验证共用同一个整体预算）+ `BORROW_PROBES_PER_CALL`(3) ×（PUT + 单次验证 ≤4 秒）+ 收尾 PUT ⇒ **≈ ≤24 秒**，取 25。PUT 按正常毫秒级计、Clash 挂起不在预算内（次数按受影响槽数累加，见 D18）。丢包只造得出「网关 TCP 连不上」那条快路（每次快探 ≤3 秒），「网关活着、隧道卡住」那一段演练量不到，靠代码里的 `timeout` 兜住 + 两条假件单测（`sentinel::resi` 判原上游那次、`slots` 借用后那次）。
- 判据②' 事件文案说「无可用出口」且**指明终态指向本槽 IP**（按 `slots --json` 的 `ip`，没体检过退回 `host:port`，与 `resi::ip_of` 同口径），且**不出现**「已临时切到」。判据②'③' 只在**候选探得完**时成立：池里上游条数 > `BORROW_PROBES_PER_CALL` + 1 时，哨兵会合法地停在一条没探过的候选上并报「未确认」，脚本开跑前按 `DRILL_PROBES_PER_CALL`（缺省 3）自查，超了打 FATAL 退 2、一条丢包规则都不插（不是缺陷，换单端口模式）。
- 判据③' 该槽的 `active_upstream_id`（= runtime 的 `current_upstream_id`）== 本槽自己的上游、`borrowed=false`。
- 判据④' 删掉丢包规则后，≤ `DRILL_BACK_WAIT` 内该槽回环出网 IP 回到演练前那个（本模式里 selector 一直在本槽，不需要等切回轮数）。
- **演练覆盖面的记录（验收必填，2026-09-14 审查第 5 条）**：验收时逐次写清「**哪台机器**（ssh 别名）、**池里几条上游**、**实际跑的是单端口还是 `--all-ports`**」。`--all-ports` 只在池里上游条数 ≤ `BORROW_PROBES_PER_CALL` + 1 = 4 时能跑（超了脚本 FATAL 退 2、一条规则都不插），所以**生产池一旦 ≥5 条上游，判据①'②'③' 在真机上永远量不到**，「无可用出口」这条路的证据就只剩假件单测（`slots::tests::{slots_whose_candidates_were_already_proven_dead_go_home_without_reprobing, the_verify_budget_is_per_call_so_a_spent_budget_reports_unconfirmed, a_verification_that_outruns_its_budget_keeps_the_candidate_and_says_so}`、`sentinel::resi::tests::{with_no_candidate_reachable_the_event_says_so_and_the_slot_goes_home, a_judging_probe_that_outruns_its_budget_is_treated_as_unavailable}`、`sentinel::run::tests::with_the_whole_gateway_down_the_event_lands_within_the_no_exit_sla`）。**不许**把它写成「已在真机覆盖」——写「未在真机跑过（池里 N 条上游 > 4），证据为上列单测」。顺带在真机记一次**借用后验证的真实耗时**，作为 D18「4 秒是否偏紧」（审查第 4 条）的依据。
- 原有单端口模式与四条判据**保持不变**；`--self-test` 加三个场景：`--all-ports` 全过（含两条丢包规则都删净），「全池不通却报『已临时切到』」⇒ 判据②' 必须 FAIL（修复前那个缺陷的回归位），以及 `DRILL_PROBES_PER_CALL=0` ⇒ 前置自查 FATAL 退 2 且不插规则。stub 的两条上游改成同一网关的不同端口（10007 / 10008）并带 `ip` 字段，`--all-ports` 因此会插 2 条规则，stub 按规则条数分辨两种模式。
...
| 要求 | 落点 |
|---|---|
| (1) 5 秒一轮（演练后改 2 秒，D17）、`--after-cursor`、cursor 存 runtime 重启续读、首次从现在不回放、`-o json`、按 `managed_units` 枚举 relay / hysteria* / xray / caddy、解析成 (unit, ts, message) 交纯函数；Host 原语 + FakeHost 可编程 | T1（原语、解析、FakeHost 队列）、T7（`tick` 的游标 / since / 节流落盘、`units()`） |
| (2) relay 连接错误 60 秒 ≥3 条（演练后连接类改 ≥2 条，D17） ⇒ 按 tag 反查上游 ⇒ 带外探测（复用 health 的探测函数）⇒ 失败则按 §5.6 借用（复用借用语义、防抖 / 手动锁定）+ 告警「IP X 不可达，槽 i 已临时切到 Y」 | T2（签名）、T4（`probe_quick` 复用 `probe_reachable`；`borrow_now` 复用 `rank_healthy` / pin 语义 / `put_slot`）、T5（`on_upstream_error`）、T7（tag → uuid、派发） |
