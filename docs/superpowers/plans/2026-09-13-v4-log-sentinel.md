...
**前置（缺一不跑）**：
1. 文末「待主理人确认的一处口径」（切回轮数）已有裁决。判据④ 的 660 秒只在维持 D6 时有依据；若裁决为严格 3 轮，先按裁决改实现，再把 `DRILL_BACK_WAIT` 的缺省值改为 `3 × 120 + 60 = 420`，然后才跑。
...
| (6) scripts/ops 真机演练：iptables 只针对该上游 IP:端口、trap 兜底恢复；≤15 秒记事件、该槽借用、回环出网 IP 改变、恢复后切回；`--self-test` 用 stub | T10（「恢复后 3 轮内」按 D6 落成「恢复后第 4 轮巡检，脚本上限 660 秒」，见下方待确认项） |
...
**待主理人确认的一处口径**：主理人原话「恢复后 3 轮内切回」。现有语义下带外确认不可达的 IP 被立即判不健康（D6），恢复要巡检先连续 2 轮探通才重新算健康（第 2 轮同时计切回第 1 轮），再攒满 `SLOT_BACK_ROUNDS = 3` 轮——即**恢复后第 4 轮巡检（约 8 分钟）切回**；演练脚本按 660 秒判。若要严格「3 轮」，可把 `mark_unhealthy` 换成「只借不改健康位」（代价：借用后的第一轮巡检会把还没恢复的 IP 当成「恢复第 1 轮」），或在切回计数里把迟滞那 2 轮算进去——两者都改 §5.6 的既有语义，本计划不做，等裁决。**裁决须在 Task 10 Step 6（真机演练）之前拿到**：判据④ 的 660 秒门槛只在维持 D6 时成立；若裁决为严格 3 轮，按裁决改完实现后把 `DRILL_BACK_WAIT` 的缺省值改为 `3 × 120 + 60 = 420`。
