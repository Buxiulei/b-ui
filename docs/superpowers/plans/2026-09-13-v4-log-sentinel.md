...
- **D18 借用后带外验证与「无可用出口」的终态（2026-09-14）**：见 D7 末尾与 Task 10 的 2026-09-14 修订。三条口径：① 借用后用与判「原上游坏」**同一个** `probe_quick` 验证刚切过去的那条，探不通即记不健康换下一个（上限 2 个，延迟算式见 `BORROW_MAX_TRIES` 的文档）；② 验证**逐条按 `host:port`**，绝不按网关主机归组——生产池里多条上游是同一网关的不同静态端口、每个端口一个出口 IP，归组会把整组判死、直接砸掉「住宅 IP 连不通立刻分配新 IP」这个功能，也会让演练判据②③必然失败；③ 候选全不通时把 selector 放回本槽自己的上游并如实报「候选 X、Y 均不可达，已放回本槽 IP Z，当前无可用出口」，连这次 PUT 都失败时把这一层也说出来。处置延迟判据因此拆成两条：有可用出口 ≤15 秒、无可用出口 ≤25 秒。
...
- 判据①' 首条 relay 错误 → 事件 ≤ `DRILL_NOEXIT_SLA`（新常量，缺省 25 秒）：轮询 ≤2 + 原上游快探 ≤3 + `BORROW_MAX_TRIES`(2) ×（Clash PUT ≤2 + 候选快探 ≤3）+ 收尾 PUT ≤2 ≈ 17 秒，留出等第 2 条错误的余量取 25。
- 判据②' 事件文案说「无可用出口」且**指明终态指向本槽 IP**（按 `slots --json` 的 `ip`，没体检过退回 `host:port`，与 `resi::ip_of` 同口径），且**不出现**「已临时切到」。
...
- 原有单端口模式与四条判据**保持不变**；`--self-test` 加两个场景：`--all-ports` 全过（含两条丢包规则都删净），以及「全池不通却报『已临时切到』」⇒ 判据②' 必须 FAIL（修复前那个缺陷的回归位）。stub 的两条上游改成同一网关的不同端口（10007 / 10008）并带 `ip` 字段，`--all-ports` 因此会插 2 条规则，stub 按规则条数分辨两种模式。
