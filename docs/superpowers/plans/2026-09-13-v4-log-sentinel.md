...
- 判据①' 首条 relay 错误 → 事件 ≤ `DRILL_NOEXIT_SLA`（新常量，缺省 25 秒）：等第 2 条错误 ≤5 + 轮询 ≤2 + **原上游快探 ≤ `health::QUICK_PROBE_BUDGET_SECS` 4 秒**（与验证共用同一个整体预算）+ `BORROW_PROBES_PER_CALL`(3) × 单次验证 ≤4 秒 ⇒ **≤23 秒**，取 25。Clash PUT 不进算式（本机毫秒级，次数按受影响槽数累加，见 D18）。丢包只造得出「网关 TCP 连不上」那条快路（每次快探 ≤3 秒），「网关活着、隧道卡住」那一段演练量不到，靠代码里的 `timeout` 兜住 + 两条假件单测（`sentinel::resi` 判原上游那次、`slots` 借用后那次）。
