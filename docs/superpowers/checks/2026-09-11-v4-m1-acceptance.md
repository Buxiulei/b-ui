...
| 项 | 归属 | 怎么验 |
|---|---|---|
| 每个现有用户三种订阅与 v3 逐项相同 | P0 的 golden 测试 + P2 的订阅端点任务 | 本机 `cargo test -p bui-schema --test golden_subscription`；bwg-rick 上 P2 合并后把该用户的订阅取下来与 v3 抓取的样本逐行 `diff` —— 路径末段就是凭据，**不要手敲进命令行**（`ps` 会泄露、还落进 shell history），先取 token 再经 `-K -` 的 stdin 传：<br>`tok=$(python3 -c 'import json,sys;print(next(u.get("sub_token","") for u in json.load(open("/opt/b-ui/state.json"))["users"] if u["username"]==sys.argv[1]))' alice)`<br>`printf 'url = "http://127.0.0.1:8080/api/sub/%s"\n' "$tok" \| curl -fsS -K - \| base64 -d` |
