...
| 分流策略 | 选择性：OpenAI / Google / Claude 走住宅，其余直出 | 速度最快（大部分流量不走额外跳）；住宅带宽消耗小；这几个站点恰好是最需要住宅 IP 的场景 |
...
选择性分流后，只有住宅规则命中的流量（OpenAI / Google / Claude）走额外一跳，其余（YouTube 下载、普通网页、大文件）全部直出 VPS 跑满速。实测住宅流量占比通常 < 15%。
...
1. 一键安装时回答 Y 并粘贴有效凭据 → 安装结束后访问 `openai.com` 看到的出口 IP 是住宅 IP。
2. 一键安装时粘贴**错误**凭据 → 校验失败提示，安装继续完成，住宅 IP 未启用。
3. 启用住宅 IP 后访问 `baidu.com`（不在分流列表）→ 出口 IP 仍为 VPS 机房 IP（直出）。
4. 启用住宅 IP 后访问 `openai.com` / `claude.ai` / `gemini.google.com` → 出口 IP 为住宅 IP。
