...
**预案边界**：哨兵只做「探测 → 借用 / 重试 / 告警」，不改 state 里的池成员；只挪每槽的 `slot-<i>-pool`，不动全局 `resi-pool`（`dns_resi` 的 detour 用它；故障 IP 恰好是全局选择时由下一轮巡检切走）；按槽借用与巡检的 `drive_slots` 互斥；替换 IP 仍由管理员在面板/CLI 执行，替换后 §5.6 的重分配自动完成。告警渠道：面板 + `bui status` + `bui incidents`；外部通知（Telegram/Webhook）只留 `Notifier` 接口，本期不做。**处置延迟判据分两条**（都从首条 relay 连接错误算到事件的 `at`，事件在预案做完后才盖时间戳）：**有可用出口 ≤15 秒** = 等第 2 条错误 ≤5 秒（并发连接几乎同时报错，串行时最多一次中继拨号超时——sing-box 缺省 5 秒，relay 出站不设 `connect_timeout`）+ 轮询 ≤2 秒 + **判原上游快探 ≤ `QUICK_PROBE_BUDGET_SECS` 4 秒**（预算）+ Clash PUT + 首候选的验证（同一个预算；网关通、隧道正常 ⇒ 一次 GET < 1 秒）⇒ **≈ ≤12 秒**；**无可用出口 ≤25 秒** = ≤5 + ≤2 + ≤4 + `BORROW_PROBES_PER_CALL`(3) ×（PUT + 验证 ≤4）+ 收尾 PUT（放回本槽）⇒ **≈ ≤24 秒**（验证那一项要取到三个**满额**预算得落在三个不同的槽上：同一个槽只要撞满一次预算就保留那个候选并停下，往下试的 `Dead` 必然 < 4 秒 —— 单槽只能逼近，方向是安全的高估）。**PUT 按正常毫秒级计**：它是本机 Clash API（单次上限 `CLASH_TIMEOUT_SECS` = 2 秒），**Clash 自己挂起不在这两条预算内**；次数也不是常数而按**受影响槽数**累加（8 槽全压在故障 IP 上时约 12 次：第一个槽最多 4 次加 1 次放回，其余各 1 次），毫秒级下合计仍 < 1 秒。**两条 SLA 都依赖两次快探各自的那个整体时限**（2026-09-14 审查第 1 条：原先只有借用后那次有 `timeout`，判原上游那次裸奔，用生产参数实测单次 28.4 秒，两条 SLA 在最常见的故障形态上必破）：丢包只造得出「网关 TCP 连不上」那条快路，「网关活着、隧道卡住」那一段量不到，上界只能由代码里的 `timeout` 保证（`sentinel::resi` 与 `slots` 各有一条假件单测）。
