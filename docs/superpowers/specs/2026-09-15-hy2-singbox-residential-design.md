...
日期：2026-09-15。状态：**主理人已拍板方向（不再讨论要不要做），spec 待主会话终审**。
...
- **`counter` 是兼容段的下线判据**：`bui status` 打印「兼容段 40001-40007 最近命中 N 次（自 <时刻>）」；**连续 30 天为 0** 才 `bui set hy2-resi-compat off`（删那两条规则 + 防火墙收口），下线前在 CHANGELOG 与面板事件提前 30 天通知。4.0 发出的按槽订阅只有 `40000+i` 这一个端口会失效（切片本来就在整段里），所以兼容段只为它存在。
...
- ⚠️ **顺带发现的既有缺陷（不在本期范围，但必须记下）**：生产上 apernet 建的是**原生 nft 表 `hysteria_*`**，而 `portjump.rs` 只扫 `iptables -t nat -S` / `ip6tables -t nat -S` 找 `HYSTERIA-PR-*` 链（`portjump.rs:47-50,87-113`）。两者对不上 ⇒ 在这些主机上 `bui hy2-prestart` 对 apernet 的 nft 残留很可能是 **no-op**，「`Chain already exists` 崩溃循环」在生产上的实际形态与 `portjump.rs` 假定的不同。4.1 把住宅路径上的 apernet 拿掉，这个缺陷只剩**直连实例**还暴露着。列入 §12 第 14 项先核实（`nft list tables` + 一次 SIGKILL 复现），确认后作为独立改动（给 `portjump` 加 nft 分支），**不塞进本期**。
- **`nft` 二进制是否预装未核实**（§12 第 2 项）：`bui install` / `upgrade` 的依赖检查增加 `nftables` 包（缺则 apt/dnf 装）；`which nft` 为假时自检 FAIL、watchdog 告警 `nft_missing`，此时 `:40000` 仍可直连、只是跳跃失效。退路（不进本期）是改用 iptables-nft 自建链。
...
**新单元正文**（`units.rs` 的 `resi_unit_text(index)` 改为无参 `resi_unit_text()`）：照今天 relay 单元的形状（`units.rs:244-265`，同款 sing-box 进程），`ExecStart={bin}/sing-box run -c {base}/hy2-residential.json`、`ExecStartPre=-{bin}/bui nft apply`、`After=network-online.target b-ui-relay.service`（保留：出站指向 relay）、`Restart=always` / `RestartSec=3`、`LimitNOFILE=1048576`、`Nice=-5`、`LogRateLimitIntervalSec=10s` + `LogRateLimitBurst=200`（relay 已有，对 §11 #10 的 `deny` 噪音正好有用）。内存沿用住宅档 `MemoryHigh=300M` / `MemoryMax=500M`（`units.rs:339-340`，实测 65 MB 有充足余量）；删掉 apernet 专属的 `Environment=HYSTERIA_LOG_LEVEL=warn` 与 `GOMEMLIMIT=200MiB`（与 relay 单元一致：relay 不设 GOMEMLIMIT），删掉 `LimitNPROC=512`（sing-box 线程模型与 apernet 不同，按 relay 档走）。
...
- **`id`** = `r` + 三位十进制（`r000`…`r511`）。**ASCII、稳定、只在本机内部用**：门的 tag `gate-<id>`、Clash API 路径、日志匹配都按它——生产用户名是中文，不能进 URL 路径。
...
**池大小 = 2 × 用户上限**：`size_for(n) = clamp(ceil16(2 × n), POOL_MIN = 32, POOL_MAX = 512)`，`n` = 有住宅权益且开 hysteria2 的用户数（仓库里没有独立的用户上限常量，用户数就是基数；容量始终 ≥ 2 倍，留一倍给增长）。128 凭据 + 128 门 + 128 规则的形状本机实测内存不涨、`check` 0.04 s；**512 是上限而非实测值**（§12 第 8 项）。
...
**裁决：不开 `cache_file`。** sing-box 重启后每个 selector 回到 `default` = `deny`，由 b-ui 重放真实门位。
...
- `SlotRes` 收缩为 `{ index, relay_port }`：`hy2_port` / `stats_port` / `hop` 三个字段，连同 `hop_slice`、`slot_span`、`HY2_STATS_RESI_BASE`（`slots.rs:19`）与 `resources_of` 一起删，只留 `resources(index) -> SlotRes`。`indices` / `sorted` / `fallback_index` / `index_of_user` / `sync_slots` / `least_loaded` / `assign*` / `migrate_unassigned` / `users_of_slot` **保留原签名**（relay、xray 与对账枚举仍按槽走）。**全部调用点**（漏一个就编译不过或显示错端口）：
...
3. > `bui-c check` **不会**自动重拉订阅（check.rs/update.rs 不取节点、不存 token）⇒ 服务端换槽/改段/开混淆后 bui-c 用户要手动 `import --sub -`。B 的目标之一就是让「换槽不再需要刷订阅」。
   ...
   ⇒ 这正是 §1.2 目标 2 的价值所在：4.1 之后换槽 / 增删 IP **不再产生任何需要重导的理由**。`slots.rs` 里那句「要等下一次订阅更新（`bui-c` 的每日 timer…）」的注释与事实不符，随本改动删除。仍需人工重导的只剩两种：rotate（今天已是如此）与**兼容段下线时**那一代「4.0 多槽、槽 ≥ 1」的导入。
...
- `HY2_CONFIGS`（`:34-37`）只留 `("hysteria-server", "config.yaml")`：孤儿链自愈只对 apernet 直连有意义。`coupling.facts` 里「端口跳跃自愈只覆盖 2 个实例（已知缺口）」那条缺口**随住宅实例一起消失**（但见 §2.4 的 ⚠️：直连那一支的 nft/iptables 错配要另案核实）。
...
| 阶段 | 内容 | 验收判据 |
|---|---|---|
| **0 · staging 补验**（0.5–1 天，tizi sidecar 复用同一套隔离端口 `:40100` / `50001-53000` / `inet bui_sidecar`） | 跑 §12 剩下的项：三种客户端矩阵（第 1 项）、`nft` 跨发行版（2）、自建二进制的 v2ray_api 与标签 / 可重现 / 体积（3–6）、QUIC 空闲字段名（7）、512 凭据（8）、证书热加载（9）、`interrupt` 对 UDP（10）、`deny` 日志速率（11）、sniff 传域名（12）；采 §8.1 的八条日志夹具 | 三种客户端各 30 分钟 `connected` 计数 = 1；自建二进制 `sing-box version` 的 `Tags` ⊇ 官方 + `with_v2ray_api`，`grpcurl` 打通 `QueryStats`；夹具八条齐 |
...
| **4 · 生产升级** | 先 `bwg-rick`：`bui upgrade` 到 `v4.1.0-rc1`（对账停旧槽单元、起新单元、写含兼容段的 nft 表；**全员不刷新订阅**），观察 24 小时；再 `bwg-tizi`；稳定 30 天且兼容段计数为 0 后宣布下线日期 | 三种订阅 sha 对每个用户不变；`hysteria-residential` `NRestarts` 只涨升级那一次；主理人用**手里未刷新**的 v2rayN 订阅连住宅 HY2 30 分钟无重连；`bui upgrade --rollback` 演练一次并过 §9.1 全部断言 |
...
| 4 | 门位重放失败 ⇒ 全员被拒（fail-closed） | §3.4：不开 `cache_file`（陈旧放行比短暂拒绝更糟）、复用 relay 的 15 秒退避、失败告警 + 60 秒安全网收敛；可用性包线与今天 `auth_http` 相同 | 设计裁决，代价已写明 |
...
| 10 | `deny` 出站的拒绝行是噪音：被封用户每个请求打一行 | 哨兵忽略 `outbound/socks[deny]`；单元带 `LogRateLimitIntervalSec=10s` + `Burst=200`；§12 第 11 项量速率，超过 relay 今天量级就把 `deny` 换成本机 blackhole 端口或降 `log.level` 为 warn | 阶段 0 |
| 11 | `nft` 二进制在某些发行版缺失 | 安装 / 升级装 `nftables` 包；缺失时 `:40000` 仍可直连、跳跃失效并告警；退路 iptables 不进本期 | §12 第 2 项 |
| 12 | v2ray_api 的 proto 包名与 Xray 不同 | §12 第 3 项：以源码为准 vendor，tonic 两份客户端互不影响 | 阶段 0 |
| 13 | 吞吐无数据 | 两者同属 quic-go 家族、服务端都走 BBR（`ignore_client_bandwidth`），评估也明确无公开基准 ⇒ **不作为 A/B 判据**；真要数字就在阶段 0 用临时机 iperf3 双向测 | **不在生产机重试**（§12 第 15 项） |
...
2. **`nft` 跨发行版**：Ubuntu 22.04 / 24.04、Debian 12、CentOS Stream 9 上 `which nft` / `nft --version`、`inet` 族 nat 链对 `redirect` 的支持、`nft -f` 单事务「declare + flush + define」是否原子；记最低可用版本。
...
8. **凭据池上限**：512 凭据 + 512 门 + 512 规则的空载 RSS 与 `check` 耗时（128 已实测不涨，512 是推断）。
...
11. **`deny` 日志速率**：一个被封用户持续请求 10 分钟，`journalctl` 行数/分钟（§11 #10 的判据）。
...
9. **新计划** `docs/superpowers/plans/2026-09-15-v4.1-hy2-singbox.md`（用 `superpowers:writing-plans`），按 §10 五阶段拆任务。
