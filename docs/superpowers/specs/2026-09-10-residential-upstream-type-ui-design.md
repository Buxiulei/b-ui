...
- 按钮"校验并添加"：点击后禁用并显示"正在连接上游校验，通常 30 秒内（最长 60 秒）…"（服务端添加路径超时 60s：helper 先 `flock -w 30` 再最坏两轮 10s 探测）；成功 → toast、列表刷新、该行显示类型徽标与出口 IP；失败 → 现有错误框显示 helper 的错误原文（含 R8 的端口白名单提示）。
