...
- **出路是重新导入并切换**：菜单 `[3]` 导入完会问「切换到新导入的 …？」，这条路上答 y 就是唯一的出路；命令行用户重新导入后要自己 `sudo bui-c list` 找到新增的那条住宅节点，再 `sudo bui-c switch <它的名字>`。
- 旧节点可以留着不管，也可以用菜单 `[6]` 或 `sudo bui-c delete <名字>` 删掉。
