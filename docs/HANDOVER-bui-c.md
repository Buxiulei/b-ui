...
- `/api/nodes/<token>`（公开）：节点集合 + 分流规则，`bui-schema::nodes::nodes_for` 与 `SplitRules` 直接序列化。客户端不需要知道槽位概念，只按载荷连。
- **服务端 4.1 之后住宅 HY2 节点是 `40000` + 整段 `41000-50000`，与槽位无关、每个用户一样，已导入的节点无需重做**（`bui-c` 不用重导，v2rayN 也不用刷新订阅）：住宅侧换成一个 sing-box 入站 + 静态凭据池，整段跳跃由服务端自管的 `table inet bui` REDIRECT 进那个单一端口，所以增删住宅上游 / `assign` / `rebalance` 都不再改任何人手里的端口或区间 —— 4.0.x 那套「换槽后必须重新获取订阅」的机制已退役。
  **兼容段下线前需要重导的只有一类**：4.0 多槽时代导入过**槽 ≥ 1** 的住宅 HY2 节点（端口落在 `40001`..`40007`）的客户端。它们现在靠服务端的 4.0 兼容段（`40001-40007` 同样 REDIRECT 到 `40000`）活着；服务端会在连续 30 天零命中 + 提前 30 天通知之后用 `bui set hy2-resi-compat off` 关掉那两条规则，届时这批节点直接连不上。判别方法：`bui-c list` / 菜单里住宅 HY2 节点的端口不是 `40000` 的，重新导入一次即可（凭据没变的节点原地更新、不改名）。
- 住宅 HY2 的**密码不是 `hy2_password`**（4.1 起）：服务端发的是该用户那条池凭据 `{name}:{secret}`（迁移用户的 `name` = 用户名、`secret` = 当时的 `hy2_password`，所以旧订阅逐字不变；4.1 期间新建或 rotate 过的用户拿到的是 `r017:<22 字符随机>` 这种）。客户端照原样填进 `auth` 即可，不要自己拼。
- **住宅 HY2 的「已连接但打不开网页」是服务端的正常语义**（4.1，spec §6）：到期 / 封禁 / 停用的用户，门被切到 `deny` ⇒ 握手照旧成功、客户端显示已连接，但每个请求都被拒。`bui-c check` 的连通性探测会失败而节点看起来「在线」，错误文案别写成「服务器不可达」。直连节点仍是建连时即被拒。
...
- **轮换凭据（面板「重置订阅链接与凭据」/ `POST /api/users/<用户名>/rotate`）会同时换订阅 token、HY2 密码与 VLESS UUID**：已部署的 `bui-c` 手里的节点凭据当场失效，`bui-c.timer` 每分钟一次 `check` 会一直判失败、按退避不停重启 sing-box，而更新源（面板 `/api/nodes/<旧 token>`）也 404 ⇒ **无法自愈，必须人工重新导入一次**（面板复制新链接 → `bui-c import --panel … ` 或菜单 [3]）。运维在轮换某个用户前要先知道他有没有 Linux 客户端。
  **4.1 起「重新导入」还不够，必须连带切换**（2026-09-17 主会话裁决：维持现状、把后果写清楚，spec `2026-09-15-hy2-singbox-residential-design.md` §7.4 第 3 条）：rotate 连住宅凭据的 `name` 一起换（用户名 → 池里的 `rNNN`），而 `profiles::same_account` 比的就是 Hysteria2 的 `username` ⇒ 住宅 HY2 是**另一个账号**，重新导入只会**新增**一条、旧的留在原地而且还是当前节点；旧凭据的门已被服务端切到 `deny` ⇒ 那条旧节点**显示已连接但所有请求被拒**。rotate 同时也换 VLESS UUID ⇒ **融合权益的用户 Reality 两条也各新增一条**（Reality 的账号判据是 uuid），旧的两条连不上，所以新增的不止一条。**「切换」不能只指着导入完那一问**：它问的是**第一个**新节点，而节点顺序是 Reality直连 → Reality住宅 → HY2直连 → HY2住宅 ⇒ 融合权益时答 y 切到的是 Reality 直连、住宅出口被静默丢掉。出路是菜单 [1] 选节点或 `sudo bui-c switch <新的住宅 HY2 节点名>`。**新节点名的准确形态请以客户端侧的命名规则为准**（服务端侧的文案与 spec 都没写死，麻烦在本节补一条准确形态供运维照抄）。服务端侧已把这几句写进 rotate 回包的 `notice` 与面板的二次确认 / 成功提示；**客户端侧不加「rotate」字样**（客户端分不清一条新节点是 rotate 来的还是用户自己新加的）。后续「只换 secret、保留 name」的提议已被否：改 `secret` 就要改 `hy2-residential.json` 的 `users[]`，那份文件写盘即重启住宅入站，等于为一个人打断全体住宅用户。
...
### 服务端 4.1 之后的 rotate：对 bui-c 来说是换了一个账号

4.1 的住宅 HY2 凭据来自凭据池，节点里的 `username` 取凭据的 `name`。迁移过来的用户 `name` 就是用户名，所以升级到 4.1 时只有端口与跳跃段变，按账号匹配能原地更新。但**管理员 rotate 之后，住宅凭据换成池里一条空闲凭据，`name` 变成 `rNNN`**（例如 `alice` 变 `r017`）。这是 4.1 零重载设计的必然结果：rotate 只在已经写进配置的凭据之间搬人，而空闲凭据的 `name` 全是 `rNNN`；要保留原名就得重写住宅配置、重启入站，把全体住宅用户的连接一起掐掉（服务端裁定，见服务端 4.1 spec §7.4 第 3 条）。

所以在 bui-c 眼里，rotate 之后的住宅节点**是另一个账号**：

- 重新导入**不会**原地更新旧的住宅节点，而是**新增**一条；旧的那条留在列表里。
- 新节点起名 `<前缀>-hy2-resi`。前缀是面板 `/api/nodes` 回包里的用户名净化后的结果（面板链接不论末段是用户名还是 token，都先走 `/api/nodes`，回包里填的是用户名）；净化后为空（中文用户名）或根本没有用户名（粘贴节点链接导入、面板接口取不到而退回订阅路径）时用节点主机名，即 `panel.example.com-hy2-resi` 这种形态。与旧节点撞名时按「被占就加序号」改成 `-2`（`-2` 也被占就 `-3`，依次往后），并打一行「节点名 <旧名> 已被另一个账号占用，新节点命名为 <新名>」，**实际名字以这一行为准**；旧节点前缀不同（例如旧的是洗过 token 名的 `panel.example.com-hy2-resi`、新的按 ASCII 用户名起 `alice-hy2-resi`）就不撞名、不打这一行，也没有序号。
- 旧节点对应的凭据已被服务端切到拒绝：在下一次服务端重写配置之前，它会**显示已连接，但所有请求都被拒绝**；重写之后才变成握手失败。如果它正好是当前节点，症状就是「连上了，什么都打不开」。
- **出路是重新导入，然后明确切到新的住宅 HY2 节点**。菜单 `[3]` 导入完那句「切换到新导入的 …？」**只问一个节点**：排在最前的新节点，顺序是面板下发的 Reality直连、Reality住宅、HY2直连、HY2住宅（活动节点被上面「只升不降」挡下、同账号的另一条被更新时，问的是那一条「切换到 …？」，排在所有新节点前面）。融合权益用户 rotate 之后 Reality 两条也是新账号（见下一段），这一问问的通常是 `…-reality-direct-2`，**答 y 落不到住宅 HY2 上**。所以要用菜单 `[1]` 切换节点选中 `…-hy2-resi-<序号>`，或命令行 `sudo bui-c list` 找到它再 `sudo bui-c switch <它的名字>`。从面板导入、权益里又没开 Reality 时，这一问问的才是住宅 HY2，答 y 即可（退回订阅路径时面板来源的旧直连会被挡下另起一条，问到的就成了 `…-hy2-direct-2`）。
- 旧节点可以留着不管，也可以用菜单 `[6]` 或 `sudo bui-c delete <名字>` 删掉。删掉会记墓碑（按凭据指纹）：日后再 rotate、凭据池恰好把同一个 `rNNN` 发回给这个用户时，导入会把它当成删过的节点跳过——菜单 `[3]` 会问要不要加回来，命令行要加 `--with-deleted`。

客户端提示文案里**不写** rotate：客户端分不清一条新节点是因为 rotate 来的，还是用户本来就新加的，写进去会在正常新增时误导。rotate 的说明由服务端在面板回包、公告里点名。直连 HY2 与 Reality 的 rotate 只换密码或 uuid：直连 HY2 的 `username` 不变，**从面板导入**时按账号匹配能原地更新（活动节点来源是 v3 或粘贴、又退回订阅路径取节点时，会被上面「只升不降」挡下、另起一条，按提示切换即可）；Reality 换 uuid 等于换账号，与今天一样要重新导入并切换。
