...
#     超时 5 秒）+ 哨兵轮询 ≤2 秒 + 原上游快探 ≤ health::QUICK_PROBE_BUDGET_SECS（4）秒（网关连不上
#     ≤3 秒就判不可达，这 4 秒是整个快探的**整体**预算）+ 借到那条的带外验证 ≤4 秒（同一个预算；
#     本模式下目标网关是通的 ⇒ 毫秒级）。Clash PUT 不进算式：本机 Clash API、正常毫秒级
#     （上限 CLASH_TIMEOUT_SECS 2 秒）。⇒ 本模式实测 ≈11 秒；两次快探都撞满预算时正好 15 秒
#     ——这一项可达（不是保守的高估）：判原上游那次撞满预算 ⇒ 结论「未确认」，而那一侧把它按
#     不可用处置、照样借用（隧道挂死时探测拿不到明确失败，见 health::Verdict），借到的那条
#     再撞满一次；sentinel::resi 有一条用例实测这条路 8.0 秒
...
#     花完：等第 2 条错误 ≤5 + 轮询 ≤2 + 原上游快探 ≤4 + BORROW_PROBES_PER_CALL（3）× 单次验证
#     ≤4 秒 ⇒ ≤23 秒，取 25（三个**满额**验证预算要落在三个不同的槽上：同一个槽撞满一次
#     预算就保留那个候选并停下，往下试的候选必然在 4 秒内给出明确失败 —— 单槽只能逼近，
#     方向是安全的高估）。Clash PUT 不进算式（本机毫秒级，次数还按受影响槽数累加）。
