...
/// Y，未能在 4 秒内确认可用」；判原上游那侧会多借一次兄弟 IP（吵闹；`mark_unhealthy` 之后巡检
/// 要先连续 [`super::OK_TO_HEALTHY`]（2）轮探通重新判健康、再攒满
/// [`super::SLOT_BACK_ROUNDS`]（3）轮，恢复后第 4 轮才切回，约 8 分钟），换来「隧道挂死」
/// 这条路上仍然 15 秒自愈。要不要放宽，等 bwg-tizi 上实测一次真实借用验证的耗时再定（计划
/// D18 记了这一项）。
