...
提示 `同一账号还有 1 个节点与服务端这次给的端口或凭据不一致：…`，在菜单 [3] 里导入会多问一句
...
IP 后区间按槽位切开，服务端会点名需要重新拉订阅的用户（`bui residential add` / `assign` / `rebalance` / `remove`
四条路径与面板「+ 添加代理」都打这份名单）；不重新导入的话，客户端每 30 秒会跳到已经划给别的住宅实例的端口上，那个实例
没有这条连接的状态、把包直接丢掉，表现为反复断联（**不是**从别的住宅 IP 出去），所以这类机器也要重新导入一次。
