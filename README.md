...
**当前版本**: v4.1.2（Rust 单二进制；版本号唯一来源是根 `Cargo.toml` 的 `[workspace.package] version`）
...
**增删住宅上游、`bui residential assign` / `rebalance` 不在这张单子上**（4.1 起）：住宅 HY2 只有
`40000` 这一个监听端口、整段 `41000-50000` 由 `table inet bui` 送进去，端口与跳跃区间与槽位无关，
所以改槽只换出口 IP，不动任何人手里那份订阅。4.0.x 那套「必须重新获取订阅」的名单机制已随之退役。
...
重新导入按**账号**原地更新（4.0.2 起）：账号 = 节点类型 + 服务器地址 + 凭据主体（HY2 认用户名，
Reality 认 uuid）。同一账号的端口变了（住宅槽位换了）或 HY2 密码变了（重置过凭据），重新导入都原地更新
原来那条、名字不变，打一行 `更新节点 alice-hy2-resi：端口 40003 → 40000`；跳跃区间、混淆密码按服务端覆盖；
当前节点不变（改到的是当前节点时自动重新生效）；删掉过的节点默认跳过。名字不规范的也一样按账号认：从 v3
迁来沿用 v3 目录名的（例如 `hysteria2-<数字>`）、4.0.0 用 token 链接导入的 `<32 位 token>-hy2-resi`、因重名
带 `-2` 后缀的、先粘贴节点链接后来改从面板导入的，都不再另起新节点。

下面两种情况仍会另起一个新节点，旧的留在列表里：

- **住宅 HY2 重置过凭据（HY2 密码变了）**（4.1 起端口不会再因换槽而变）：只有名字正是这次导入会起的
  ...
  **4.1 起「重置订阅链接与凭据」（rotate）换掉的住宅 HY2 也算另一个账号**：rotate 连住宅凭据的
  名字一起换（迁移用户原来那条叫他自己的用户名，新发的叫 `rNNN`），而 `bui-c` 的 HY2 账号看的就是
  用户名 ⇒ 住宅节点走的是下面 Reality 那条路：屏幕上提示「节点名 …… 已被另一个账号占用」，新节点
  另起一个名字（例如 `<用户名>-hy2-resi-2`），**旧的留在原地、而且还是当前节点**——服务端已经把它的
  门切到 `deny`，它会显示已连接但所有请求被拒。rotate 同时也换 VLESS UUID，所以 Reality 那两条同样
  各另起一个新节点（旧的两条连不上）。新增的不止住宅这一条，而**导入完那一问只切第一个新节点**
  （融合权益时通常是 Reality 直连），所以 rotate 之后一定要照下面「先切后删」**明确切到新的住宅
  HY2 节点**。HY2 直连不受影响（换的只是密码、用户名没换，原地更新）。
- **重置过凭据后的 Reality 节点**：uuid 换了，算另一个账号，新节点与旧节点同名时命名为 `<原名>-2`
  （屏幕上提示「节点名 … 已被另一个账号占用」）。旧节点已经连不上，要清理。
- **粘贴的链接（或订阅）碰上面板导入的节点、或碰上当前节点，而参数不是全等**：例如粘的是凭据轮换之前留下
  的旧链接。这时不覆盖它们，按新节点导入并打一行说明出路（同一账号下已经有别的、不受保护的副本时不另起，只更新那一条，菜单会改问「切换到那一条？」）——面板那条「要更新它请从面板重新导入」，当前节点
  那条「确认新节点能用后可以切换过去」。另外，手里的链接是直连还是住宅只能从备注认（含「直连」或「住宅」
  才算数），备注被改过的链接只认同端口的那一条，端口不同不算同一账号。

升级到 4.0.2 之前攒下的同账号重复节点（例如换过槽位留下的 `alice-hy2-resi-2`）不会自动合并：命令行导入只
提示 `同一账号还有 1 个节点：…`，在菜单 [3] 里导入会多问一句
`要合并吗？[y/N]`（默认 N），答 y 才并进本次已更新的那条。
...
住宅 HY2 节点存的一律是端口 `40000` + 整段跳跃区间 `41000-50000`，池里有几个住宅 IP 都一样，**服务端 4.1 之后
已导入的节点无需重导**。唯一的例外是 **4.0 多槽时代导入过槽 ≥ 1 的住宅节点**（端口落在 `40001`..`40007`）：
它们现在靠兼容段活着，要在兼容段下线前重新导入一次。4.0.x 那种「换槽后客户端每 30 秒跳到别的实例的端口上、
反复断联」的形状不再存在（整段只进一个进程）。

**节点名里的订阅 token**：4.0.0 用 `bui-c import --sub <token 链接>` 导入过的机器，节点名是 `<32 位 token>-<类型>`，
带着订阅 token（一长串十六进制），`bui-c list`、菜单、`bui-c status` 都会显示，截图就会泄露。4.0.2 起这类名字
**导入一次就自动改掉**：随便从面板或订阅导入一次，名字换成 `<主机>-<类型>`（被占用则加 `-2`），每条打一行
`节点 0123…-hy2-resi 已改名为 panel.example.com-hy2-resi`，端口没变时也改；改名不重启代理、不影响当前节点是谁，
也不用先删后导。给人看的输出（`bui-c list`、`status`、菜单、删除确认等）另外一律把 token 打码成前 4 位加省略号
（`0123…-hy2-resi`），所以改名之前截图也不再泄露完整 token；`--json` 与 `profiles.json` 不打码。**按旧 token 名
写的脚本（`bui-c switch <旧名>`、`bui-c delete <旧名>`）会失效**，改名后照新名字改一次；打码后的名字也不能直接
敲进 `switch` / `delete`，请在菜单里按编号选，或从 `bui-c list --json` 取原名。

已经截过图、或把带完整 token 的输出发给过别人的，token 本身已经外泄：到面板里对这个用户「重置订阅链接与凭据」，
再用新链接重新导入（HY2 按同一账号原地更新；Reality 换了 uuid 算另一个账号，会另起新节点，按上面的先切后删把
旧的删掉）。
...
| 40000 | UDP | Hysteria2 住宅（**唯一**监听端口，一个 sing-box 入站） |
| 41000-50000 | UDP | Hysteria2 住宅的端口跳跃：由 b-ui 自管的 `table inet bui` 整段 REDIRECT 到 `40000`，这一段本身不监听 |
| 40001-40007* | UDP | 4.0 兼容段（可选，默认开）：4.0 多槽时代发出的按槽端口，同样 REDIRECT 到 `40000`，让那批还没刷订阅的客户端继续可用。连续 30 天零命中 + 提前 30 天通知后用 `bui set hy2-resi-compat off` 下线 |
...
- **重置（怀疑链接泄露时）**：面板配置弹窗里的「重置订阅链接与凭据」（`POST /api/users/<用户名>/rotate`）
  ...
  Linux 客户端**导入完还要明确切换到新的住宅 HY2 节点**：4.1 的 rotate 连住宅凭据的名字一起换，
  重新导入只会新增新节点、旧的留在原地当当前节点，而它会显示已连接但所有请求被拒；
  导入完那一问只切**第一个**新节点（融合权益时通常是 Reality 直连），要用菜单 [1] 或
  `sudo bui-c switch <新的住宅 HY2 节点名>`（详见上面「服务端改了节点参数之后」）。
